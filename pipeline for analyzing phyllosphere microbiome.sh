...
### 1.1. 元数据/实验设计 metadata
    ...
    csvtk -t stat result/metadata_raw5.txt
    # 元数据至少3列，首列为样本ID(SampleID)，结尾列为描述(Description)
    # cat查看文件，-A显示符号，"|"为管道符实现命令连用，head显示文件头，-n3控制范围前3行
    cat -A result/metadata_raw5.txt | head -n3
    # windows用户结尾有^M，运行sed命令去除，再用cat -A检查
    sed 's/\r//;s/ //g' result/metadata_raw4.txt > result/metadata5.txt
    cat -A result/metadata5.txt | head -n3
...
### 1.2. 测序数据 sequencing data
    ...
    ls -sh seq/
    zless seq/AHFDL1_1.fq.gz | head -n4 
    # 每行太长，指定查看每行的1-60个字符
    zless seq/AHFDL1_1.fq.gz | head | cut -c 1-60
    # 统计测序数据，依赖seqkit程序
    seqkit stat seq/AHFDL1_1.fq.gz
    # 批量统计测序数据并汇总表
...
## 3. 切除引物与质控 Cut primers and quality filter
    ...
    time vsearch --fastx_filter temp/all.fq \
      --fastq_stripleft 20 --fastq_stripright 20 \
...
### 4.2 聚类OTU/去噪ASV Cluster or denoise
    ...
    #6s, 1748 good, 2453 chimeras, 序列百万条可能需要几天/几周
...
### 5.1 生成特征表
    ...
    # id(0.97)：97%相似度比对78.49%序列，1m10s(更高数据使用率，更快)
    time vsearch --usearch_global temp/filtered.fa \
      --db result/raw/otus.fa \
      --id 0.97 --threads 8 \
    	--otutabout result/raw/otutab.txt 
    # 10401278 of 10729414 (96.94%)可比对
...
### 6.3. 筛选高丰度菌 Filter by abundance
    ...
    Rscript ${db}/script/otu_mean.R --input result/otutab.txt \
      ...
      --output result/otutab_meanPSN.txt
    # 结果为全部和各组均值
    head -n3 result/otutab_meanPSN.txt
    ...
    awk 'BEGIN{OFS=FS="\t"}{if(FNR==1) {for(i=3;i<=NF;i++) a[i]=$i; print "OTU","Group";} \
        ...
        result/otutab_meanPSN.txt > result/alpha/otu_group_existPSN.txt
    head result/alpha/otu_group_existPSN.txt
    cut -f 2 result/alpha/otu_group_existPSN.txt | sort | uniq -c
...
## 9. 有参定量特征表
    ...
    # 比对率69.8%, 5636384 / 8075009 mapped to OTUs
    ...
    # 比对率9017262 of 10729414 (84.04%), 1核30m, 12核7m
...
### 1.1 Alpha多样性箱线图
    ...
    Rscript ${db}/script/alpha_boxplot.R --alpha_index richness \
      --input result/alpha/vegan.txt --design result/metadata4.txt \
      --group Group --output result/alpha/ \
      --width 200 --height 150
    # 省份间比较，多样性指数可选richness chao1 ACE shannon simpson invsimpson
    Rscript ${db}/script/alpha_boxplot.R --alpha_index richness \
      --input result/alpha/vegan.txt --design result/metadata5.txt \
      --group PS --output result/alpha/ \
      --width 200 --height 150
    # PSN省份间比较，多样性指数可选richness chao1 ACE shannon simpson invsimpson
    Rscript ${db}/script/alpha_boxplot.R --alpha_index richness \
      --input result/alpha/vegan.txt --design result/metadata4.txt \
      --group PSN --output result/alpha/ \
      --width 200 --height 150
    # 使用循环绘制6种常用指数
...
### 1.3 多样性维恩图
    ...
    bash ${db}/script/sp_vennDiagram.sh \
      ...
      -a AHFDS -b AHSDS -c AHTDS \
      -w 3 -u 3 \
      -p AHFDS_AHSDS_AHTDS
    # 四组比较，图和代码见输入文件目录，运行目录为当前项目根目录
    bash ${db}/script/sp_vennDiagram.sh \
      -f result/alpha/otu_group_exist.txt \
      -a QHHS -b QDHS -c QJHS -d QWHS \
      -w 3 -u 3 \
      -p QHHS_QDHS_QJHS_QWHS
...
### 2.2 主坐标分析PCoA
    ...
    # 查看帮助
    Rscript ${db}/script/beta_pcoa.R -h
    ...
    Rscript ${db}/script/beta_pcoa.R \
      ...
      --group Group --label FALSE --width 180 --height 120 \
      --output result/beta/bray_curtis.pcoa.pdf
    # 输入文件，选择分组，输出文件，图片尺寸mm，统计见beta_pcoa_stat.txt
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadata2.txt \
      --group PS --label FALSE --width 180 --height 120 \
      --output result/beta/bray_curtis.pcoa_PS.pdf
    # PSN输入文件，选择分组，输出文件，图片尺寸mm，统计见beta_pcoa_stat.txt
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadata3.txt \
      --group PSN --label FALSE --width 180 --height 120 \
      --output result/beta/bray_curtis.pcoa_PSN.pdf
    # 添加样本标签 --label TRUE
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadata2.txt \
      --group PS --label TRUE --width 180 --height 120 \
...
### 2.3 限制性主坐标分析CPCoA
    ...
    Rscript ${db}/script/beta_cpcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadata.txt \
      --group Group --label TRUE --width 180 --height 120 \
...
### 3.1 堆叠柱状图Stackplot
    ...
    Rscript ${db}/script/tax_stackplot.R \
      ...
      --output result/tax/sum_g.stackplotPaired
    # PS 修改颜色--color ggplot, manual1(22), Paired(12) or Set3(12)
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadata2.txt \
      --group PS --color Paired --legend 12 --width 181 --height 119 \
      --output result/tax/sum_g.stackplotPS
    # PSN 修改颜色--color ggplot, manual1(22), Paired(12) or Set3(12)
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadata4.txt \
      --group PSN --color manual1 --legend 21 --width 181 --height 119 \
      --output result/tax/sum_g.stackplotPSN5
    # PS 修改颜色--color ggplot, manual1(22), Paired(12) or Set3(12)
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadata2.txt \
      --group PS --color manual1 --legend 18 --width 181 --height 119 \
      --output result/tax/sum_g2.stackplotPS18
    
     Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadata.txt \
      --group Group --color paired --legend 12 --width 181 --height 119 \
      --output result/tax/sum_g1.stackplotPaired
...
### 1.1 差异比较
    ...
    Rscript ${db}/script/compare.R \
      --input result/otutab.txt --design result/metadata3.txt \
      --group PSN --compare ${compare} --threshold 0.1 \
...
### 1.4 曼哈顿图
    ...
    # PS 显示完整图例，再用AI拼图，Genus
    bash ${db}/script/compare_manhattan.sh -i result/compare/${compare}.txt \
       -t result/taxonomy.txt \
       -p result/tax/sum_g.txt \
       -w 183 -v 149 -s 7 -l 10 -L Genus \
       -o result/compare/${compare}.manhattan.g.PSN.pdf
    # 显示完整图例，再用AI拼图，class
    bash ${db}/script/compare_manhattan.sh -i result/compare/${compare}.txt \
       -t result/taxonomy.txt \
       -p result/tax/sum_f.txt \
       -w 183 -v 149 -s 7 -l 10 -L Family \
       -o result/compare/${compare}.manhattan.f.legend.pdf

### 1.5 单个特征的绘制

    # 筛选显示差异ASV，按KO组丰度降序列，取ID展示前10
    awk '$4<0.05' result/compare/AH_D-AH_H.txt | sort -k7,7nr | cut -f1 | head
    # 差异OTU细节展示
    Rscript ${db}/script/alpha_boxplot.R --alpha_index ASV_27 \
      --input result/otutab.txt --design result/metadata3.txt \
      --transpose TRUE --scale TRUE \
      --width 250 --height 200 \
      --group PS --output result/compare/feature_ 
    ...
    # PS 差属细节展示
    Rscript ${db}/script/alpha_boxplot.R --alpha_index Microbacterium \
      --input result/tax/sum_g.txt --design result/metadata3.txt \
      --transpose TRUE \
      --width 250 --height 200 \
      --group PSN --output result/compare/feature_
      
      # 指定某列排序：按科丰度均值All降序
    csvtk -t sort -k All:nr result/tax/sum_f.txt | head
    # 差属细节展示
    Rscript ${db}/script/alpha_boxplot.R --alpha_index Microbacteriaceae \
      --input result/tax/sum_f.txt --design result/metadata.txt \
      --transpose TRUE \
      --width 250 --height 200 \
...
### 2.2 绘制扩展柱状图和表
    ...
    Rscript ${db}/script/compare_stamp.R \
      --input result/stamp/tax_6Genus.txt --metadata result/metadata2.txt \
      --group PS --compare ${compare} --threshold 0.1 \
...
## 3. Bugbase细菌表型预测
    ...
    Rscript ${bugbase}/bin/run.bugbase.r -L ${bugbase} \
      -i gg/otutab.txt -m metadata3.txt -c PSN -o bugbase/
...
## 3. FAPROTAXS元素循环
    ...
    # 设置工作目录
    wd=/mnt/c/A20230801/result/faprotax/
    mkdir -p ${wd} && cd ${wd}
    # 设置脚本目录
...
## 6. 文件批量命名
    ...
    # 转换行尾换行符
    sed -i 's/\r//' metadata1.txt
    # 检查手动命名列2是否唯一
    cut -f 2 metadata1.txt|wc -l
    cut -f 2 metadata1.txt|sort|uniq|wc -l
    # 如果两次结果一致，则命名非冗余
    # 可选移动mv，复制cp，硬链ln，或软链ln -s
    # 此处使用复制cp
    # awk '{system("cp "$1" "$2)}' metadata1.txt
    #  此处使用复制cp
    awk '{system("cp raw/"$1".R1.raw.fastq.gz seq/"$2"_1.fq.gz")}' metadata1.txt
    awk '{system("cp raw/"$1".R2.raw.fastq.gz seq/"$2"_2.fq.gz")}' metadata1.txt
