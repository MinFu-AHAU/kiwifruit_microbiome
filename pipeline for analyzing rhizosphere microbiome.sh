...
### 1.1. 元数据/实验设计 metadata
    ...
    sed 's/\r//;s/ //g' result/metadata_raw.txt > result/metadata.txt
...
### 4.1 序列去冗余 Dereplicate
    ...
    vsearch --derep_fulllength temp/filtered.fa \
      --minuniquesize 120 --sizeout --relabel Uni_ \
...
### 4.2 聚类OTU/去噪ASV Cluster or denoise
    ...
    #6s, 19676 good, 382 chimeras, 序列百万条可能需要几天/几周
    usearch -unoise3 temp/uniques.fa -minsize 60 \
      -zotus temp/zotus.fa
...
### 5.1 生成特征表
    ...
    time vsearch --usearch_global temp/filtered.fa \
      ...
      --id 0.97 --threads 8 \
    	--otutabout result/raw/otutab.txt 
    # 11590403 of 16214015 (71.48%)可比对
...
### 2.2 主坐标分析PCoA
    ...
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadataSC.txt \
      --group PSN --label FALSE --width 180 --height 120 \
      --output result/beta/bray_curtis.pcoa_PSNSC.pdf
    # 添加样本标签 --label TRUE
    Rscript ${db}/script/beta_pcoa.R \
      --input result/beta/bray_curtis.txt --design result/metadata.txt \
      --group Group --label TRUE --width 180 --height 120 \
      --output result/beta/bray_curtis.pcoa.label PS2.pdf
...
### 3.1 堆叠柱状图Stackplot
    ...
    Rscript ${db}/script/tax_stackplot.R \
      ...
      --group PSN --color ggplot --legend 7 --width 89 --height 59 \
    ...
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadata.txt \
      --group PSN --color Paired --legend 12 --width 181 --height 119 \
      --output result/tax/sum_g.stackplotPSN3
    # PS 修改颜色--color ggplot, manual1(22), Paired(12) or Set3(12)
    Rscript ${db}/script/tax_stackplot.R \
      --input result/tax/sum_g.txt --design result/metadataAH.txt \
      --group PSN --color manual1 --legend 21 --width 181 --height 119 \
      --output result/tax/sum_g2.stackplotPSN6AH
...
### 1.5 单个特征的绘制
    ...
    awk '$4<0.05' result/compare/AHRtK-AHRtH.txt | sort -k7,7nr | cut -f1 | head
    # 差异OTU细节展示
    Rscript ${db}/script/alpha_boxplot.R --alpha_index ASV_59 \
      --input result/otutab.txt --design result/metadata.txt \
    ...
    # PS 差属细节展示
    Rscript ${db}/script/alpha_boxplot.R --alpha_index Pantoea \
      --input result/tax/sum_g.txt --design result/metadataHB.txt \
      --transpose TRUE \
      --width 250 --height 200 \
      --group PSN --output result/compare/feature_
